...
module setup_device
    ...
    integer, constant:: npx_d, npy_d, npxm1_d, npym1_d
    integer, constant:: npSolid_d,order_d
    integer, constant:: nVar_d
    integer, device, allocatable:: ij2solid_d(:,:), solid2ij_d(:,:),&
            BI_bbox_d(:,:,:), IP_bbox_d(:,:,:),ij2iSolid_d(:,:), grid_mark_d(:,:)
    
    double precision, constant:: gam_d,x0_d,y0_d,radius_d,dt_d,eta_min_d,hx_d,hy_d,&
                                 delta_dis_d
    ...
    !Cylinder
    integer, constant:: nCylinder_d
    type(Cylinder), device, allocatable:: Cylinders_d(:)
...
module kernel_function
    ...
    subroutine SSPRK_time_stepping_gpu
        ...
        !stage 1
        resid_d = 0.d0
        !call boundary_condition_gpu
        call compute_IBM_boundary_gpu
        !call compute_IBM_boundary
        !ub=ub_d
        !ub_BI = ub_BI_d
        !ub_IP = ub_IP_d
        !open(11,file='ub_BI_gpu_s1.txt')
        !do iSolid=1,npSolid
        !    write(11,*)ub_BI(:,iSolid)
        !enddo
        !close(11)
!
        !open(11,file='ub_IP_gpu_s1.txt')
        !do iSolid=1,npSolid
        !write(11,*)ub_IP(:,iSolid)
        !enddo
        !close(11)
!
        !open(11,file='ub_GP_gpu_s1.txt')
        !do iSolid = 1,npSolid
        ...
        !do ipx=1,npx
        !    do ipy=1,npy
        !        if(ieee_is_nan(resid(ipx,ipy,1)).or.&
        !           ieee_is_nan(resid(ipx,ipy,2)).or.&
        !           ieee_is_nan(resid(ipx,ipy,3)).or.&
        !           ieee_is_nan(resid(ipx,ipy,4)))then
        !            write(*,*)'stage 1'
        !            write(*,*)'ipx=',ipx,',ipy=',ipy,',isSolid=',ij2solid(ipx,ipy)
        !            write(*,*)resid(ipx,ipy,:)
        !            stop
        !        endif
        !    enddo
        ...
        !ub_BI = ub_BI_d
        !ub_IP = ub_IP_d
        !open(11,file='ub_BI_gpu_s2.txt')
        !do iSolid=1,npSolid
        !    write(11,*)ub_BI(:,iSolid)
        !enddo
        !close(11)
        !open(11,file='ub_IP_gpu_s2.txt')
        !do iSolid=1,npSolid
        !write(11,*)ub_IP(:,iSolid)
        !enddo
        !close(11)
        !open(11,file='ub_GP_gpu_s2.txt')
        !do iSolid = 1,npSolid
        !ipx=solid2ij(1,iSolid)
        !ipy=solid2ij(2,iSolid)
        !write(11,*)ub(ipx,ipy,:)
        ...
        istat = cudaEventSynchronize(stopEvent)
        ierr_ = cudaGetLastError()
        call check_cuda_error(ierr_,'f90',149)
        ...
        istat = cudaEventSynchronize(stopEvent)
        ierr_ = cudaGetLastError()
        call check_cuda_error(ierr_,'f90',157)

        !ub=ub_d
        !open(11,file='ub_gpu_s2.txt')
        !write(11,*)ub(:,:,:)
        !close(11)
        !resid = resid_d
        !do ipx=1,npx
        ...
        !ub_BI = ub_BI_d
        !ub_IP = ub_IP_d
        !open(11,file='ub_BI_gpu_s3.txt')
        !do iSolid=1,npSolid
        !    write(11,*)ub_BI(:,iSolid)
        !enddo
        !close(11)
        !open(11,file='ub_IP_gpu_s3.txt')
        !do iSolid=1,npSolid
        !write(11,*)ub_IP(:,iSolid)
        !enddo
        !close(11)
        !open(11,file='ub_GP_gpu_s3.txt')
        !do iSolid = 1,npSolid
        !ipx=solid2ij(1,iSolid)
        !ipy=solid2ij(2,iSolid)
        !write(11,*)ub(ipx,ipy,:)
        ...
        istat = cudaEventSynchronize(stopEvent)
        ierr_ = cudaGetLastError()
        call check_cuda_error(ierr_,'f90',175)
        ...
        istat = cudaEventSynchronize(stopEvent)
        ierr_ = cudaGetLastError()
        call check_cuda_error(ierr_,'f90',183)
        !ub=ub_d
        !open(11,file='ub_gpu_s3.txt')
        !write(11,*)ub(:,:,:)
        !close(11)
        ...
        !ub_BI = ub_BI_d
        !ub_IP = ub_IP_d
        !open(11,file='ub_BI_gpu_s4.txt')
        !do iSolid=1,npSolid
        !    write(11,*)ub_BI(:,iSolid)
        !enddo
        !close(11)
        !open(11,file='ub_IP_gpu_s4.txt')
        !do iSolid=1,npSolid
        !write(11,*)ub_IP(:,iSolid)
        !enddo
        !close(11)
        !open(11,file='ub_GP_gpu_s4.txt')
        !do iSolid = 1,npSolid
        !ipx=solid2ij(1,iSolid)
        !ipy=solid2ij(2,iSolid)
        !write(11,*)ub(ipx,ipy,:)
        ...
        istat = cudaEventSynchronize(stopEvent)
        ierr_ = cudaGetLastError()
        call check_cuda_error(ierr_,'f90',199)
        ...
        istat = cudaEventSynchronize(stopEvent)
        ierr_ = cudaGetLastError()
        call check_cuda_error(ierr_,'f90',207)

        !ub=ub_d
        !open(11,file='ub_gpu_s4.txt')
        !write(11,*)ub(:,:,:)
        !close(11)
        !resid = resid_d
        !do ipx=1,npx
        ...
        !ub_BI = ub_BI_d
        !ub_IP = ub_IP_d
        !open(11,file='ub_BI_gpu_s5.txt')
        !do iSolid=1,npSolid
        !    write(11,*)ub_BI(:,iSolid)
        !enddo
        !close(11)
        !open(11,file='ub_IP_gpu_s5.txt')
        !do iSolid=1,npSolid
        !write(11,*)ub_IP(:,iSolid)
        !enddo
        !close(11)
        !open(11,file='ub_GP_gpu_s5.txt')
        !do iSolid = 1,npSolid
        !ipx=solid2ij(1,iSolid)
        !ipy=solid2ij(2,iSolid)
        !write(11,*)ub(ipx,ipy,:)
        ...
        istat = cudaEventSynchronize(stopEvent)
        ierr_ = cudaGetLastError()
        call check_cuda_error(ierr_,'f90',223)
        ...
        istat = cudaEventSynchronize(stopEvent)
        ierr_ = cudaGetLastError()
        call check_cuda_error(ierr_,'f90',231)

        !ub=ub_d
        !open(11,file='ub_gpu_s5.txt')
        !write(11,*)ub(:,:,:)
        !close(11)
    ...
    subroutine compute_IBM_boundary_gpu
        ...
        istat = cudaEventSynchronize(stopEvent)
        ierr_ = cudaGetLastError()
        call check_cuda_error(ierr_,'f90',263)
        !ub_BI = ub_BI_d
        !open(11,file='ub_BI_gpu.txt')
        !do iSolid=1,npSolid
        !    write(11,*)ub_BI(:,iSolid)
        !enddo
        !close(11)
        ...
        istat = cudaEventSynchronize(stopEvent)
        ierr_ = cudaGetLastError()
        call check_cuda_error(ierr_,'f90',272)
        !ub_IP=ub_IP_d
        !open(11,file='ub_IP_gpu.txt')
        !do iSolid=1,npSolid
        !    write(11,*)ub_IP(:,iSolid)
        !enddo
        !close(11)
        ...
        istat = cudaEventSynchronize(stopEvent)
        ierr_ = cudaGetLastError()
        call check_cuda_error(ierr_,'f90',281)
        !ub = ub_d
        !open(11,file='ub_GP_gpu.txt')
        !do iSolid = 1,npSolid
        !    ipx=solid2ij(1,iSolid)
        !    ipy=solid2ij(2,iSolid)
        !    write(11,*)ub(ipx,ipy,:)
    ...
    attributes(global) subroutine compute_ub_BI_gpu
        ...
        integer:: i1,j1,i2,j2,i3,j3,i4,j4,iCylinder
        double precision:: xGP, yGP, xBI, yBI
        double precision:: x1,y1,x2,y2,x3,y3,x4,y4,xc,yc,r
        ...
        if(iSolid<=npSolid_d)then
            ...
            iCylinder = ij2solid_d(iGP,jGP)
            xc=Cylinders_d(iCylinder)%xc
            yc=Cylinders_d(iCylinder)%yc
            r=Cylinders_d(iCylinder)%r
            ...
            dis = dsqrt((xBI-xc)**2.d0 + (yBI-yc)**2.d0)
            !dis = dsqrt((xBI-x0_d)**2.d0 + (yBI-y0_d)**2.d0)

            if(dis<=epsilon)then
                write(*,*)'Error: no grid points inside the solid region'
                stop
            endif
    ...
    attributes(global) subroutine compute_fluxF()
        ...
        if(i<=npx_d+1 .and. j<=npy_d)then
            ...
            !call reconstruct_gpu_dis(fp_stencil,Rmat,Rinv,fluxp,mark)
            ...
            !call reconstruct_gpu_dis(fm_stencil,Rmat,Rinv,fluxm,mark)
    ...
    attributes(device) subroutine reconstruct_gpu(u_stencil,Rmat,Rinv,uf)
        use setup_device,only:nVar_d, eta_min_d
        implicit none
        double precision :: u_stencil(4,5),Rmat(4,4),Rinv(4,4),flux(4),&
                            v_stencil(4,5),vp0(4),vp1(4),vp2(4),vf(4),tau5
        ...
        integer :: i, d0, d1, d2
        double precision:: Cr = 0.24d0, xi = 0.001d0, epsilon
    ...
    attributes(device) subroutine reconstruct_gpu_dis(u_stencil,Rmat,Rinv,uf,mark)
    use setup_device,only:nVar_d, eta_min_d
        implicit none
        double precision :: u_stencil(4,5),Rmat(4,4),Rinv(4,4),flux(4),&
                            v_stencil(4,5),vp0(4),vp1(4),vp2(4),vf(4),tau5
    ...
    !
        ...
        uf = matmul(Rmat,vf)
    end subroutine reconstruct_gpu_dis

    attributes(device) subroutine reconstruct_gpu_dis_ad(u_stencil,Rmat,Rinv,uf,mark)
    use setup_device,only:nVar_d, eta_min_d,order_d
        implicit none
        double precision :: u_stencil(4,5),Rmat(4,4),Rinv(4,4),flux(4),&
        ...
        double precision::tol = 1d-1
        integer :: i, d0, d1, d2
        double precision:: Cr = 0.24d0, xi = 0.001d0, epsilon, alpha1 = 9.d0, alpha2 = 8.2d0
...
program Cylinder
    ...
    integer :: ipx,ipy,argc,iostat,device_count,device_id,istat,iCy
    ...
    !---------------------
    nCylinder = 3
    allocate(Cylinders(nCylinder))
    Cylinders(1)%xc=0.6d0
    Cylinders(1)%yc=1.0d0
    Cylinders(1)%r = 0.1d0
    Cylinders(2)%xc=1.d0
    Cylinders(2)%yc=1.4d0
    Cylinders(2)%r = 0.1d0
    Cylinders(3)%xc=1.d0
    Cylinders(3)%yc=0.6d0
    Cylinders(3)%r = 0.1d0
    !---------------------------

    casename='MultiCylinder_'
    ! 格式化文件夹路径为 "月.日.时.分"
    ...
    do iCy=1,nCylinder
        write(*,*)Cylinders(iCy)%xc, Cylinders(iCy)%yc, Cylinders(iCy)%r
    enddo
    ...
    write(333,*)'Multi-Cylinder_TENO5opt_GPU ','npx = ',npx,',npy = ',npy,',dt = ', dt
    do iCy=1,nCylinder
        write(*,*)'Cylinder ',iCy,Cylinders(iCy)%xc,Cylinders(iCy)%yc,Cylinders(iCy)%r
    enddo
    write(333,*)
    close(333)
    ...
    do while (ctime<=tf)
        ...
        if(iter==1 .or.mod(iter,10)==0 .or.iter==maxiter)then
            print *,'iter=',iter,'time=',ctime,'resid=',residglob,'dis=',delta_dis 
        endif
        
        if (iter==maxiter .or. mod(iter,NFC)==0) then

            call cpu_time(timeStop1)
            filename = "Info.dat"
            write(full_path,'(A,"/",A)') trim(folder_path), trim(filename)
            open(333,file=full_path,position='APPEND')
            write(333,*)'time/iter(s): ',(timeStop1-timeStart1)/500
            close(333)
...
subroutine gpu_allocate
    ...
    allocate(Cylinders_d(nCylinder),stat=ierr); call gerror_report(ierr)
    Cylinders_d=Cylinders
...
subroutine compute_geometry
    use setup
    implicit none
    integer:: ipx, ipy, npFluid, iSolid, iCylinder
    integer::i1,j1,i2,j2,i3,j3,i4,j4
    double precision::x1,y1,x2,y2,x3,y3,x4,y4
    double precision:: tmp, x0, y0, radius
    double precision::xGP, yGP, xIP, yIP, xBI, yBI
    ...
    do ipy = 1,npy
        do ipx = 1,npx
            ...
            do iCylinder = 1, nCylinder
                tmp = dsqrt((xmsh(ipx)-Cylinders(iCylinder)%xc)**2.d0 +&
                            (ymsh(ipy)-Cylinders(iCylinder)%yc)**2.d0)
                
                if(tmp <= Cylinders(iCylinder)%r)then
                    npSolid = npSolid + 1
                    ij2solid(ipx,ipy) = iCylinder
                endif

                if(tmp >= Cylinders(iCylinder)%r - delta_dis .and.&
                   tmp <= Cylinders(iCylinder)%r + delta_dis)then
                    grid_mark(ipx,ipy) = 1
                endif
    ...
    do iSolid = 1, npSolid
        ...
        iCylinder = ij2solid(ipx,ipy)

        call compute_IP_BI_Circle(xGP,yGP,xIP,yIP,xBI,yBI,iCylinder)
...
subroutine compute_IP_BI_Circle(xGP,yGP,xIP,yIP,xBI,yBI,iCylinder)
    use setup
    implicit none
    integer, intent(in) :: iCylinder
    double precision, intent(in) :: xGP,yGP
    double precision, intent(out) ::  xIP,yIP,xBI,yBI
    double precision:: xGPn, yGPn
    double precision:: x0, y0, radius, dis, k
    double precision:: epsilon = 1d-20

    x0 = Cylinders(iCylinder)%xc
    y0 = Cylinders(iCylinder)%yc
    radius = Cylinders(iCylinder)%r

    xGPn = xGP
    yGPn = yGP

    dis = dsqrt((xGPn-x0)**2.d0 + (yGPn-y0)**2.d0)
...
end subroutine compute_IP_BI_Circle
...
subroutine Cylinder_Initialize
    use setup, only:gam, npx, npy, ub, xmsh, ymsh,uInlet
    implicit none
    integer:: ipx, ipy
    double precision:: rho, u1, u2, pr, dis
...
end subroutine Cylinder_Initialize

subroutine boundary_condition_gpu
    use setup
    use setup_device
    use kernel_function
    implicit none
    ...
    !bound_type(i)=1 -> inlet
    !              2 -> sym
    !              3 -> period
    !              4 -> outflow
    !              5 -> wall

    !do i=1,4 ! 1->xmin, 2->xmax, 3->ymin, 4->ymax
!
    !    select case(bound_type(i))
!
    !        case(1)
    !            call inlet_condition(i)
    !        
    !        case(2)
    !            call sym_condition(i)
!
    !        case(3)
    !            call period_condition(i)
!
    !        case(4)
    !            call outflow_condition(i)
!
    !        case(5)
    !            call wall_condition(i)
    !            
!
    !    end select
!
...
end subroutine boundary_condition_gpu
...
subroutine write_solu_tecplot
    ...
    write(11,*) 'TITLE = "Cylinder Problem"'
    write(11,*) 'VARIABLES = "X" "Y" "RHO" "U" "V" "P"'
    write(11,12) 'ZONE NODES=',npx*npy,',ELEMENTS=',(npx-1)*(npy-1),&
            ',DATAPACKING=POINT,ZONETYPE=FEQUADRILATERAL,SOLUTIONTIME=',ctime
    12  FORMAT(A11,I10,A10,I10,A57,F6.3)
    ...
    do ipy = 1,npy
        do ipx = 1,npx
            ...
            u = ub(ipx,ipy,2)/rho
            v = ub(ipx,ipy,3)/rho
            pr= (ub(ipx,ipy,4) - 0.5*rho*(u**2+v**2))*(gam-1.d0)



            write(11,*) xmsh(ipx),ymsh(ipy),rho,&
            u,v,pr
...
!subroutine write_solu_tecplot_IBM
!    use setup
!    implicit none
!
!    character*6 :: decimal
...
!
!    write(decimal,'(f6.3)') ctime-int(ctime)
!    write(filename,'(a,i7.7,a4,a)')'TEC_IBM',int(ctime),decimal(3:6),'.dat'
!    write(full_path,'(A,"/",A)') trim(folder_path), trim(filename)
!    open(11,file=full_path)
!    write(11,*) 'TITLE = "Cylinder Problem"'
!    write(11,*) 'VARIABLES = "X" "Y" "RHO" "U" "V" "P"'
...
!            pr= (ub(ipx,ipy,4) - 0.5*rho*(u**2+v**2))*(gam-1.d0)
!
!            if(dsqrt((xmsh(ipx)-x0)**2.d0+(ymsh(ipy)-y0)**2.d0) < radius)then
!                rho = 0.d0
!                u = 0.d0
!                v = 0.d0
!                pr = 0.d0
...
!    close(11)
!    
!end subroutine write_solu_tecplot_IBM
...
subroutine write_solu_tecplot_IBM_binary
    ...
    do ipy=1,npy
        do ipx = 1,npx
            ...
            do iCy=1,nCylinder
                d_tmp = dsqrt((xmsh(ipx)-Cylinders(iCy)%xc)**2.d0+(ymsh(ipy)-Cylinders(iCy)%yc)**2.d0)
                if(d_tmp < Cylinders(iCy)%r)then
                    q(ipx,ipy,1)= 0.d0
                    q(ipx,ipy,2) = 0.d0
                    q(ipx,ipy,3) = 0.d0
                    q(ipx,ipy,4) = 0.d0
	...
	write(11) 0                     
	write(11) 6                    ! Number of variables (NumVar) in the datafile
...
!subroutine compute_IBM_boundary
!    use setup
!    use setup_device
!    use, intrinsic::ieee_arithmetic
!    implicit none
...
!    double precision:: xGP, yGP, xBI, yBI, xIP, yIP
!    double precision:: x1,y1,x2,y2,x3,y3,x4,y4
!    double precision:: phi(nVar,4)
!    double precision:: rhoGP,uGP,vGP,prGP
!    double precision:: rhoBI,uBI,vBI,prBI
!    double precision:: rhoIP,uIP,vIP,prIP
!    integer::ipx,ipy
!
...
!    do iSolid = 1, npSolid
!
!        iGP = solid2ij(1,iSolid)
!        jGP = solid2ij(2,iSolid)
!        
!        xGP = xmsh(iGP)
!        yGP = ymsh(jGP)
!        xBI = BI2xy(1,iSolid)
...
!        
!
!        i1 = BI_bbox(1,1,iSolid)
!        j1 = BI_bbox(2,1,iSolid)
!        i2 = BI_bbox(1,2,iSolid)
!        j2 = BI_bbox(2,2,iSolid)
!        i3 = BI_bbox(1,3,iSolid)
!        j3 = BI_bbox(2,3,iSolid)
!        i4 = BI_bbox(1,4,iSolid)
!        j4 = BI_bbox(2,4,iSolid)
!
!        phi(:,1) = ub(i1,j1,:)
!        phi(:,2) = ub(i2,j2,:)
!        phi(:,3) = ub(i3,j3,:)
!        phi(:,4) = ub(i4,j4,:)
!
!        call linear_interpolation(xBI,yBI,BI_Ainv(:,:,iSolid),phi(:,:),ub_BI(:,iSolid))
!        call modify_BI_velocity(xBI,yBI,ub_BI(:,iSolid))
!
!        !if(ieee_is_nan(ub_BI(1,iSolid)).or.&
!        !   ieee_is_nan(ub_BI(2,iSolid)).or.&
!        !   ieee_is_nan(ub_BI(3,iSolid)).or.&
!        !   ieee_is_nan(ub_BI(4,iSolid)))then
!        !    write(*,*)'iSolid=',iSolid
!        !    write(*,*)xBI,yBI
!        !    stop
!        !endif
...
!        !write(*,*)xIP,yIP
!
!        i1 = IP_bbox(1,1,iSolid)
!        j1 = IP_bbox(2,1,iSolid)
!        i2 = IP_bbox(1,2,iSolid)
!        j2 = IP_bbox(2,2,iSolid)
!        i3 = IP_bbox(1,3,iSolid)
!        j3 = IP_bbox(2,3,iSolid)
!        i4 = IP_bbox(1,4,iSolid)
!        j4 = IP_bbox(2,4,iSolid)
!
!
!        if(ij2solid(i1,j1)==1)then
!            phi(:,1) = ub_BI(:,ij2iSolid(i1,j1))
!        else
!            phi(:,1) = ub(i1,j1,:)
!        endif
!
!        if(ij2solid(i2,j2)==1)then
!            phi(:,2) = ub_BI(:,ij2iSolid(i2,j2))
!        else
!            phi(:,2) = ub(i2,j2,:)
!        endif
!
!        if(ij2solid(i3,j3)==1)then
!            phi(:,3) = ub_BI(:,ij2iSolid(i3,j3))
!        else
!            phi(:,3) = ub(i3,j3,:)
!        endif
!
!        if(ij2solid(i4,j4)==1)then
!            phi(:,4) = ub_BI(:,ij2iSolid(i4,j4))
!        else
!            phi(:,4) = ub(i4,j4,:)
!        endif
!
!        call linear_interpolation(xIP,yIP,IP_Ainv(:,:,iSolid),phi(:,:),ub_IP(:,iSolid))
!
!    enddo
!    !!!=============================
!
...
!        yIP = IP2xy(2,iSolid)
!
!        call compute_original_var(ub_BI(:,iSolid),rhoBI,uBI,vBI,prBI)
!        call compute_original_var(ub_IP(:,iSolid),rhoIP,uIP,vIP,prIP)
!
!        !rhoGP = 2.d0*rhoBI-rhoIP
!        !prGP  = 2.d0*prBI-prIP
!        rhoGP = rhoIP
...
!        !vGP = -vIP
!
!        call compute_conserved_var(rhoGP,uGP,vGP,prGP,ub(iGP,jGP,:))
!
!    enddo
!
!    ub_d = ub
...
!    !stop
!
!end subroutine compute_IBM_boundary
!
!!subroutine modify_BI_velocity(xBI,yBI,ubBI)
!!    use setup
!!    use, intrinsic::ieee_arithmetic
!!
!!    implicit none
!!    double precision, intent(in) :: xBI,yBI
!!    double precision, intent(inout) :: ubBI(nVar)
!!    double precision:: dis,enx,eny,etx,ety,epsilon = 1d-20
!!    double precision:: rhoBI,uBI,vBI,prBI 
!!    double precision:: unBI,utBI
!!    logical::isNan
!!    dis = dsqrt((xBI - x0)**2.d0 + (yBI - y0)**2.d0)
!!
!!    if(dis <= epsilon)then
!!        write(*,*)'Error: no grid points inside the solid region'
!!        stop
!!    else
!!        
!!        call compute_original_var(ubBI,rhoBI,uBI,vBI,prBI)
!!        
!!        enx = (xBI - x0)/dis
!!        eny = (yBI - y0)/dis
!!        etx = -eny
...
!!        vBI = unBI*eny + utBI*ety
!!
!!        call compute_conserved_var(rhoBI,uBI,vBI,prBI,ubBI)
!!
!!
!!    endif
!!    
!!end subroutine modify_BI_velocity
!
!subroutine linear_interpolation(x,y,Ainv,phi,ub_out)
!    use setup, only:nVar
!    implicit none
!    integer::i
!    double precision, intent(in) :: x,y,Ainv(4,4),phi(nVar,4)
!    double precision, intent(out) :: ub_out(nVar)
!    double precision:: a, b, c, d
!
!    do i = 1,nVar
!
...
!
!subroutine compute_original_var(cv,rho,u,v,pr)
!    use setup, only: gam, nVar
!    implicit none
!    double precision, intent(in) :: cv(nVar)
!    double precision, intent(out) ::  rho,u,v,pr
!
!    rho = cv(1)
!    u   = cv(2)/rho
!    v   = cv(3)/rho
!    pr  = (cv(4) - 0.5d0*rho*(u**2.d0+v**2.d0))*(gam - 1.d0)
!    
!end subroutine compute_original_var
!
!subroutine compute_conserved_var(rho,u,v,pr,cv)
!    use setup, only: gam, nVar
!    implicit none
!    double precision, intent(in) :: rho,u,v,pr
!    double precision, intent(out) :: cv(nVar)
!
!    cv(1) = rho
!    cv(2) = rho*u
!    cv(3) = rho*v
!    cv(4) = pr/(gam-1.d0) + 0.5d0*rho*(u*u + v*v)
!
!end subroutine compute_conserved_var