...
module setup
    ...
    !Cylinder
    double precision:: x0, y0, radius, delta_dis,eta_min
...
module kernel_function
    ...
    use setup_device,only:nVar_d, eta_min_d
        ...
        integer :: i, d0, d1, d2
        double precision:: Cr = 0.2d0, xi = 0.001d0, epsilon, alpha1 = 9.d0, alpha2 = 8.2d0
...
program Cylinder
    ...
    integer :: ipx,ipy,argc,iostat,device_count,device_id,istat
    ...
    write(333,*)'Cylinder_TENO5opt_GPU ','npx = ',npx,',npy = ',npy,',dt = ', dt,&
    'grid type = ', gridType
    write(333,*)'Cylinder information: center x = ', x0, ', center y = ',y0,&
    'radius = ',radius
    write(333,*)
    close(333)
    ...
    do while (ctime<=tf)
        ...
        if (iter==maxiter &
            .or. dabs(ctime-0.45d0)<1d-8 &
            .or. dabs(ctime-2.d0)<1d-8 &
            .or. dabs(ctime-3.05d0)<1d-8 &
            .or. dabs(ctime-3.5d0)<1d-8 &
            .or. dabs(ctime-5.d0)<1d-8) then
...
subroutine gpu_allocate
    ...
    !allocate(ub_d(-2:npx+3,-2:npy+3,nVar),stat=ierr); call gerror_report(ierr)
    !allocate(ub0_d(-2:npx+3,-2:npy+3,nVar),stat=ierr); call gerror_report(ierr)
    !allocate(uba_d(-2:npx+3,-2:npy+3,nVar),stat=ierr); call gerror_report(ierr)
    ...
    uInlet_d = uInlet
    !allocate(fpInlet_d(nVar), stat=ierr); call gerror_report(ierr)
    !fpInlet_d = fpInlet
    !allocate(fmInlet_d(nVar), stat=ierr); call gerror_report(ierr)
    !fmInlet_d = fmInlet
...
subroutine compute_geometry
    ...
    do iSolid = 1, npSolid
        ...
        call compute_IP_BI_Circle(xGP,yGP,xIP,yIP,xBI,yBI)
        IP2xy(1,iSolid) = xIP
        IP2xy(2,iSolid) = yIP
        BI2xy(1,iSolid) = xBI
        BI2xy(2,iSolid) = yBI
...
subroutine compute_IP_BI_Circle(xGP,yGP,xIP,yIP,xBI,yBI)
    use setup, only: x0,y0,radius
    implicit none
    double precision, intent(in) :: xGP,yGP
    double precision, intent(out) ::  xIP,yIP,xBI,yBI
    double precision:: xGPn, yGPn
    double precision:: dis, k
...
subroutine Cylinder_Initialize
    use setup, only:gam, npx, npy, ub, x0, y0, radius, xmsh, ymsh
    implicit none
    integer:: ipx, ipy
    ...
    ub(:,:,2) = rho*u1
    ub(:,:,3) = rho*u2
    ub(:,:,4) = pr/(gam-1.d0)+0.5*rho*(u1**2+u2**2)
